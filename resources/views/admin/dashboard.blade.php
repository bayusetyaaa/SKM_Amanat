                ...
                <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
                    <div>
                        ...
                        <p class="card-text text-white-50 mb-0">
                            Kelola seleksi calon anggota, konfigurasi kriteria Profile Matching, penugasan, presensi, serta penetapan divisi magang Redaksi & Konten secara objektif dan terukur.
                        </p>
                    </div>
                </div>
            </div>
                    ...
                    <tbody class="table-border-bottom-0">
                        @forelse($cakrumaTerbaru as $ca)
                            <tr>
                                ...
                                <td>
                                    <span class="fw-semibold text-heading">{{ $ca->profil->pilihan_divisi_awal ?? 'Belum memilih' }}</span>
                                </td>
                                ...
                                <td class="text-center">
                                    @php
                                        ...
                                        $isLolosSemua = ($admStatus === 'lolos' && $tesStatus === 'lolos' && $cakStatus === 'lolos' && !$hasRejected);

                                        $rec = $ca->hasilProfileMatching->firstWhere('rekomendasi', true);
                                    @endphp
                                    @if(!$isLolosSemua)
                                        <span class="badge bg-label-secondary text-muted" title="Belum lolos semua tahapan seleksi">Belum Memenuhi Syarat</span>
                                    @elseif($rec)
